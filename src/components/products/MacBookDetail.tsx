...
export default function MacBookDetail({
  initialProduct,
  currentSlug,
  baseSlug,
  urlStorage,
  ...
  // 1. Khởi tạo lựa chọn ban đầu từ variants thực tế
  useEffect(() => {
    if (!product?.variants || product.variants.length === 0) return;

    const firstVar = product.variants[0];
    const initialSt = (urlStorage || firstVar.storage || '256GB').trim().toUpperCase();
    setSelectedStorage(initialSt);
    setSelectedColor(firstVar.color || 'Space Gray');
  ...
  // 2. Danh sách dung lượng SSD thực tế có trong sản phẩm
  const storageList = useMemo(() => {
    if (!product?.variants) return [];
    const set = new Set<string>();
    product.variants.forEach((v: any) => {
      if (v.storage && String(v.storage).trim().toLowerCase() !== 'tiêu chuẩn') {
        set.add(v.storage.trim().toUpperCase());
      }
    });
    return set.size > 0 ? Array.from(set) : ['256GB', '512GB', '1TB', '2TB'];
  }, [product]);

  // 3. Danh sách màu sắc thực tế
  ...
  // 4. Xác định biến thể hiện tại theo dung lượng và màu đã chọn
  ...
  // 5. Chuyển đổi dung lượng: Chuyển URL sang slug mới
  const handleSelectStorage = (st: string) => {
    setSelectedStorage(st);
    const cleanSt = st.toLowerCase().trim();
    const newSlug = `${baseSlug}-${cleanSt}`;
    router.push(`/san-pham/${newSlug}`);
  };

  // 6. Chuyển đổi màu sắc: Giữ nguyên URL, đổi hình ảnh tại chỗ
  ...
  const imagesList: string[] = useMemo(() => {
    ...
    return product?.variants?.[0]?.images || ['https://images.unsplash.com/photo-1517336714731-489689fd1ca8?w=600'];
  ...
  const cleanProductName = product?.name
    ? product.name.replace(/\b(128GB|256GB|512GB|1TB|2TB)\b/gi, '').trim()
    : 'MacBook';

  const currentPrice = currentVariant?.price || product?.price || 28990000;
  ...
  const handleAddToCart = (redirectCart = false) => {
    ...
    addToCart({
      ...
      modelSlug: product.slug || baseSlug,
              ...
              {activeTab === 'desc' && (
                <div className="space-y-3 whitespace-pre-line text-xs md:text-sm leading-relaxed text-gray-800">
                  {product?.description ? (
                    ...
                    <p>
                      <strong className="text-gray-900">{cleanProductName}</strong> mang đến sức mạnh đột phá với dòng chip Apple Silicon M-Series thế hệ mới, thời lượng pin bền bỉ cả ngày dài và màn hình Liquid Retina sắc nét với độ chuẩn màu cực cao.
                    </p>
                  )}
                </div>
              )}
                ...
                <div className="max-w-4xl overflow-hidden rounded-xl border border-red-500 bg-white shadow-xs">
                  ...
                  <div className="divide-y divide-gray-200 text-xs md:text-sm">
                    {(Array.isArray(product?.specifications) && product.specifications.length > 0
                      ...
                      : [
                          { key: 'Màn hình', value: 'Liquid Retina / Liquid Retina XDR độ sáng 500 - 1000 nits, chuẩn màu DCI-P3' },
                          { key: 'Hệ điều hành', value: 'macOS phiên bản mới nhất tối ưu phần cứng Apple Silicon' },
                          { key: 'Vi xử lý', value: 'Chip Apple Silicon M-Series (CPU & GPU hiệu năng cao, Neural Engine 16 lõi)' },
                          { key: 'Cổng kết nối', value: 'MagSafe 3, cổng Thunderbolt / USB 4, Jack tai nghe 3.5mm cao cấp' },
                          { key: 'Camera & Âm thanh', value: 'Camera FaceTime HD 1080p, hệ thống 4 - 6 loa hỗ trợ Spatial Audio' },
                          { key: 'Pin & Sạc', value: 'Thời lượng pin ấn tượng lên đến 18 - 22 giờ liên tục | Hỗ trợ sạc nhanh MagSafe' },
                          { key: 'Thiết kế & Trọng lượng', value: 'Vỏ nhôm nguyên khối tái chế 100%, thiết kế siêu mỏng nhẹ thanh lịch' },
