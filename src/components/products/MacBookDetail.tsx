...
export default function MacBookDetail({
  ...
  const cleanProductName = product?.name
    ? product.name.replace(/\b(128GB|256GB|512GB|1TB|2TB|24GB|Tiêu chuẩn)\b/gi, '').trim()
    : 'MacBook';
                    ...
                    {colorList.map(({ color, sampleVariant }) => {
                      ...
                      return (
                        <button
                          ...
                          className={`px-3.5 py-2 rounded-lg border flex items-center gap-2.5 transition-all cursor-pointer ${
                            isSelected
