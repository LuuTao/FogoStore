...
export default function MacBookDetail({
  ...
  useEffect(() => {
    ...
    const activeSt = (urlStorage || product.variants[0]?.storage || '256GB').toUpperCase();
    ...
    fetch(`http://localhost:5000/api/products/filter?category=macbook`, { cache: 'no-store' })
      .then((r) => r.json())
      .then((resJson) => {
  ...
  const storageList = useMemo(() => {
    ...
    const merged = Array.from(new Set([...MACBOOK_STORAGES, ...existing]));
  ...
  const cleanProductName = product.name.replace(/\b(128GB|256GB|512GB|1TB|2TB)\b/gi, '').trim();
        ...
        <main className="max-w-7xl mx-auto px-4 py-6">
          ...
          <div className="mt-12 bg-white p-6 rounded-sm border border-gray-200">
            ...
            <div className="text-xs text-gray-700 leading-relaxed space-y-4 max-w-4xl">
              <p>
                <strong className="text-gray-900">{cleanProductName}</strong> mang đến sức mạnh đột phá với dòng chip Apple M-Series thế hệ mới, thời lượng pin cả ngày dài và màn hình Liquid Retina sắc nét đáp ứng mọi tiêu chuẩn đồ họa chuyên nghiệp.
              </p>
            </div>
          </div>
