...
export default function MacBookDetail({
  ...
  useEffect(() => {
    ...
    const initialSt = (urlStorage || firstVar.storage || 'Tiêu chuẩn').trim();
  ...
  // 2. Danh sách dung lượng thực tế có trong sản phẩm
  const storageList = useMemo(() => {
    if (!product?.variants) return [];
    const set = new Set<string>();
    product.variants.forEach((v: any) => {
      if (v.storage) set.add(v.storage.trim());
    });
    return Array.from(set);
  }, [product]);

  // 3. Danh sách màu sắc thực tế có trong sản phẩm
  ...
  const currentVariant = useMemo(() => {
    ...
    // Tìm biến thể khớp cả dung lượng lẫn màu sắc
  ...
  // 5. Chuyển đổi dung lượng (Cập nhật tức thì không reload trang / không đổi URL rác)
  const handleSelectStorage = (st: string) => {
    setSelectedStorage(st);
  };

  // 6. Chuyển đổi màu sắc
  ...
  const cleanProductName = product?.name ? product.name.trim() : 'MacBook';
      ...
      <div>
        ...
        <div className="w-full bg-[#f8f9fa] border-b border-gray-200 py-2 px-4 text-[11px] text-gray-600">
          <div className="max-w-7xl mx-auto flex items-center gap-1.5 truncate">
            ...
            <span>/</span>
            <span className="text-gray-900 font-semibold">{cleanProductName}</span>
          </div>
        </div>
            ...
            <div className="lg:col-span-7 space-y-4 text-xs">
              <div>
                ...
                <h1 className="text-xl md:text-2xl font-black text-gray-900 leading-snug mt-1">
                  {cleanProductName} - Chính hãng Apple VN
                </h1>
              ...
              {/* Dung lượng SSD thực tế */}
              {storageList.length > 0 && (
                <div className="space-y-1.5 pt-2">
                  <span className="font-semibold text-gray-700">Bộ nhớ lưu trữ (SSD):</span>
              ...
              {!isOutOfStock && currentVariant && Number(currentVariant.stock || 0) > 0 && (
                <div className="flex items-center gap-3 pt-2">
                  ...
                  <span className="text-[11px] text-gray-500">(Còn {currentVariant.stock} máy trong kho)</span>
                </div>
              )}

              {/* Banner Ưu đãi MacBook */}
              <div className="bg-gradient-to-r from-[#1d1d1f] to-[#3a3a3c] text-white p-3 rounded-sm flex items-center justify-between mt-2">
                <span className="font-bold text-xs">Giảm thêm 500.000đ khi mua kèm Magic Mouse & Hub chuyển đổi Type-C</span>
          ...
          <div className="mt-12 bg-white p-6 rounded-sm border border-gray-200">
            ...
            <div className="text-xs text-gray-700 leading-relaxed space-y-4 max-w-4xl">
              {product.description ? (
                ...
                <p>
                  <strong className="text-gray-900">{cleanProductName}</strong> mang đến sức mạnh đột phá với dòng chip Apple M-Series thế hệ mới, thời lượng pin cả ngày dài và màn hình Liquid Retina sắc nét.
                </p>
              )}
            </div>
          </div>
          ...
          {relatedProducts.length > 0 && (
            <div className="mt-12">
              <h3 className="text-base font-black text-gray-900 uppercase tracking-wide mb-4 flex items-center gap-2">
                ...
                <span>Các Dòng MacBook Khác Cùng Quan Tâm</span>
              </h3>
                  ...
                  return (
                    <div key={rel.id} className="bg-white rounded-sm p-3 flex flex-col justify-between shadow-xs border border-gray-200">
                      <Link href={`/san-pham/${rel.slug}`} className="w-full h-36 flex items-center justify-center p-2">
                        <img
                          src={v.images?.[0] || 'https://images.unsplash.com/photo-1517336714731-489689fd1ca8?w=300'}
                          alt={rel.name}
