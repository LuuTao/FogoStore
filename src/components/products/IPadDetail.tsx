...
export default function IPadDetail({
  ...
  useEffect(() => {
    ...
    try {
      const raw = localStorage.getItem('fogo_recent_ipads');
      const list = raw ? JSON.parse(raw) : [];
      const filtered = list.filter((i: any) => !i.slug.startsWith(baseSlug));
      const updated = [
        {
          id: product.id,
          name: `${product.name.replace(/\b(64GB|128GB|256GB|512GB|1TB|2TB)\b/gi, '').trim()} ${activeSt}`,
      ...
      localStorage.setItem('fogo_recent_ipads', JSON.stringify(updated));
  ...
  const handleAddToCart = (redirectCart = false) => {
    ...
    if (redirectCart) {
      ...
      // Kích hoạt thanh thông báo màu xanh trượt ra
      setToast({
        show: true,
      ...
      <div>
        ...
        <div className="w-full bg-[#f8f9fa] border-b border-gray-200 py-2 px-4 text-[11px] text-gray-600">
          <div className="max-w-7xl mx-auto flex items-center gap-1.5 truncate">
            ...
            <span>/</span>
            <Link href="/ipad" className="hover:text-[#d70018]">Máy tính bảng iPad</Link>
            <span>/</span>
        ...
        <main className="max-w-7xl mx-auto px-4 py-6">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-7 space-y-4 text-xs">
              ...
              {/* Banner Ưu đãi Phụ kiện iPad */}
              <div className="bg-gradient-to-r from-[#0066cc] to-[#0099ff] text-white p-3 rounded-sm flex items-center justify-between mt-2">
                <span className="font-bold text-xs">Giảm thêm 300.000đ khi mua kèm Apple Pencil & Magic Keyboard</span>
                  ...
                  return (
                    <div key={rel.id} className="bg-white rounded-sm p-3 flex flex-col justify-between shadow-xs border border-gray-200">
                      <Link href={`/san-pham/${rel.slug}`} className="w-full h-36 flex items-center justify-center p-2">
                        <img
                          src={v.images?.[0] || 'https://images.unsplash.com/photo-1544244015-0df4b3ffc6b0?w=300'}
                          alt={rel.name}
